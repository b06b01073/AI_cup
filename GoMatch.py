...
class GoMatch:
    ...
    def fit(self, train_set, test_set, optimizer, epoch, tau, unsupervised_coef, save_dir, file_name):
        best_acc = 0
        for i in range(epoch):
            print(f'Epoch [{i + 1}/{epoch}]')

            train_acc = self.train(train_set, optimizer, tau, unsupervised_coef)
            test_acc = self.test(test_set, save_dir, file_name)
    ...
    def train(self, train_set, optimizer, tau, unsupervised_coef):
        ...
        with tqdm(train_set, leave=False, dynamic_ncols=True) as pbar:
            for labeled_states, label_onehots, weak_augmented_states, strong_augmented_states in pbar:
                ...
                weak_preds = self.net(weak_augmented_states)
                ...
                unsupervised_loss = self.loss_fn_sum(weak_preds[confidence_mask].detach(), strong_preds[confidence_mask]) / unlabeled_size
